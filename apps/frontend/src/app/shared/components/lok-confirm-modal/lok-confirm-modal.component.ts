...
@Component({
  ...
  template: `
    <div class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <!-- Modal -->
      <div
        class="relative bg-white rounded-xl shadow-xl max-w-md w-full mx-4 p-6 transform transition-all"
      >
        ...
        <div class="flex gap-3">
          <button (click)="cancelled.emit()" class="flex-1 btn-secondary">
            {{ cancelLabel }}
          </button>
          <button (click)="confirm.emit()" class="flex-1 btn-danger">
            {{ confirmLabel }}
          </button>
        </div>
