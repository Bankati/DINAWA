...
@Component({
  ...
  standalone: true,
  template: `
    <div [class]="containerClasses()" class="rounded-lg p-4 flex items-start gap-3">
      <!-- Icône -->
      <div [class]="iconContainerClasses()" class="flex-shrink-0 p-1 rounded-full">
        <svg [class]="iconClasses()" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path [attr.d]="iconPath()" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"/>
        </svg>
      </div>
      ...
      <div class="flex-1">
        @if (titre) {
          <h4 [class]="titleClasses()" class="font-semibold mb-1">{{ titre }}</h4>
        }
