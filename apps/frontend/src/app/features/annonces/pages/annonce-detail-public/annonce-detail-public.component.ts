...
@Component({
          ...
          @if (annonce.photos && annonce.photos.length > 1) {
            <div class="photo-thumbs">
              @for (photo of annonce.photos; track photo; let i = $index) {
                <button class="thumb-btn" [class.thumb-active]="photoPrincipale === photo" (click)="photoPrincipale = photo">
                  <img [src]="photo" [alt]="'Photo ' + (i + 1)" class="thumb-img">
                </button>
              }
            </div>
          }
  ...
  styles: `
    ...
    .share-wa:hover { background: #F0FDF4; border-color: #25D366; }
    .share-copy { color: var(--color-text-muted); }
    .share-copy:hover { background: var(--color-primary-50); border-color: var(--color-primary); color: var(--color-primary); }
    ...
    .thumb-btn.thumb-active { border-color: var(--color-accent); }
