...
export default function GestionnaireRapportsPage() {
  ...
  const EXPORTS: { key: ExportKey; icon: string; title: string; desc: string; needsPeriod: boolean; path: string; filename: string }[] = [
    { key: 'paiements', icon: '💳', title: 'Historique des paiements', desc: 'Tous les paiements sur la période, avec locataire, bien, mode et statut.', needsPeriod: true, path: `/exports/paiements?from=${from}&to=${to}`, filename: `warah-paiements-${date}.pdf` },
    { key: 'biens', icon: '🏠', title: 'Liste des biens', desc: 'Adresses, types, statuts, loyers et charges des biens sous mandat.', needsPeriod: false, path: '/exports/biens', filename: `warah-biens-${today}.pdf` },
    { key: 'locataires', icon: '👥', title: 'Liste des locataires', desc: 'Contacts, statut de compte et bien loué de chaque locataire.', needsPeriod: false, path: '/exports/locataires', filename: `warah-locataires-${today}.pdf` },
    { key: 'rapport', icon: '📊', title: 'Rapport financier', desc: 'Synthèse mois par mois des encaissements confirmés sur la période.', needsPeriod: true, path: `/exports/rapport?from=${from}&to=${to}`, filename: `warah-rapport-${date}.pdf` },
  ];
  ...
  return (
    <div style={{ padding: '24px 28px' }}>
      <div style={{ background: 'linear-gradient(135deg, #0A2650 0%, #0F4C81 60%, #081E41 100%)', borderRadius: 14, padding: '24px 28px', marginBottom: 24, position: 'relative', overflow: 'hidden' }}>
        ...
        <h1 style={{ color: '#fff', fontSize: 22, fontWeight: 700, margin: 0 }}>Rapports & Exports</h1>
        <p style={{ color: 'rgba(255,255,255,0.65)', fontSize: 13, margin: '4px 0 0' }}>Téléchargez vos données en PDF dans votre dossier de téléchargements</p>
      </div>
      ...
      <div style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 12, padding: '16px 20px', marginBottom: 24, display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
        ...
        {[['Du', from, (v: string) => setFrom(v)], ['Au', to, (v: string) => setTo(v)]].map(([label, val, setter]) => (
          <div key={label as string} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <label style={{ fontSize: 12, color: '#6B7280' }}>{label as string}</label>
        ...
        <span style={{ fontSize: 11, color: '#9CA3AF' }}>Utilisé pour « Paiements » et « Rapport »</span>
      </div>
        ...
        {EXPORTS.map(exp => (
          <div key={exp.key} style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 12, padding: '20px 20px 18px', display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div>
              ...
              {exp.needsPeriod && <div style={{ fontSize: 11, color: '#9CA3AF', marginTop: 6 }}>Période : {from} → {to}</div>}
            </div>
            {errors[exp.key] && <div style={{ fontSize: 12, color: '#DC2626', background: '#FEF2F2', borderRadius: 8, padding: '6px 10px' }}>{errors[exp.key]}</div>}
            {done[exp.key] && !errors[exp.key] && <div style={{ fontSize: 12, color: '#059669', background: '#F0FDF4', borderRadius: 8, padding: '6px 10px' }}>✓ PDF téléchargé</div>}
            <button onClick={() => run(exp.key, exp.path, exp.filename)} disabled={loading !== null}
              style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: loading === exp.key ? '#E5E7EB' : '#0F4C81', color: loading === exp.key ? '#6B7280' : '#fff', border: 'none', borderRadius: 9, padding: '9px 0', fontWeight: 600, fontSize: 13, cursor: loading !== null ? 'not-allowed' : 'pointer' }}>
              <svg style={{ width: 15, height: 15 }} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/>
              </svg>
              {loading === exp.key ? 'Génération…' : 'Télécharger PDF'}
            </button>
          </div>
        ))}
      </div>
