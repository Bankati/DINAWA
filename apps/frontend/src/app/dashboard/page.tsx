...
function formatMoisCourt(mois: string): string {
  ...
  if (isNaN(year) || isNaN(month) || month < 1 || month > 12) return mois;
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const stale = getDashboardStale(annee);
    if (stale) {
      setKpis(stale.kpis);
      ...
      setLoadingPaiements(false); setLoadingBiens(false); setLoadingAlertes(false);
    }

    // 2. Refresh en fond — toujours, même si stale data affichée
    const onError = () => setError(true);
    getKPIs(annee).then((d) => { setKpis(d); setLoadingKPIs(false); }).catch(() => { setLoadingKPIs(false); onError(); });
    getRevenusMensuels(annee).then((d) => { setRevenus(d); setLoadingRevenus(false); }).catch(() => { setLoadingRevenus(false); onError(); });
