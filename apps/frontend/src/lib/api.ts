...
export function subscribeApiLoading(fn: (loading: boolean) => void): () => void {
  _listeners.add(fn);
  return () => _listeners.delete(fn);
}
function _notify() { _listeners.forEach(fn => fn(_pending > 0)); }
...
async function request<T>(path: string, options: RequestInit = {}, silent = false): Promise<T> {
  ...
  if (res.status === 401 && path !== "/auth/refresh" && path !== "/auth/login") {
    ...
    const isJson2 = retryRes.headers.get("content-type")?.includes("application/json");
