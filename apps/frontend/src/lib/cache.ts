...
function lsKey(key: string) { return STORAGE_PREFIX + key; }
...
export function cacheSet<T>(key: string, data: T): void {
  ...
  mem.set(key, entry);
  try { localStorage.setItem(lsKey(key), JSON.stringify(entry)); } catch { /* quota exceeded */ }
}
...
export function cacheIsStale(key: string, ttlMs: number): boolean {
  ...
  try {
    const raw = localStorage.getItem(lsKey(key));
    if (raw) { const e = JSON.parse(raw) as Entry<unknown>; return Date.now() - e.ts > ttlMs; }
  } catch { /* ignore */ }
  return true;
}
