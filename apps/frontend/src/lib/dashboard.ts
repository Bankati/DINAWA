...
function saveDashboardCache(annee: number, data: DashboardData): void {
  ...
  try { localStorage.setItem(LS_KEY, JSON.stringify({ annee, data })); } catch { /* ignore */ }
}

export function invalidateDashboardCache(): void {
  cache = null;
  if (typeof window !== 'undefined') { try { localStorage.removeItem(LS_KEY); } catch { /* ignore */ } }
}

function getData(annee: number): Promise<DashboardData> {
  if (cache && cache.annee === annee) return cache.promise;
  const promise = buildDashboard(annee).then((data) => { saveDashboardCache(annee, data); return data; });
  cache = { annee, promise };
  return promise;
}
