...
describe('SupabaseAuthGuard', () => {
  ...
  it("rejette avec 403 EMAIL_NOT_CONFIRMED si le rôle du JWT n'est pas 'authenticated'", async () => {
    // Un token Supabase d'un compte non confirmé porte role='anon', jamais
    // 'authenticated' (voir supabase-auth.guard.ts) — email_confirmed_at
    // n'est pas le champ vérifié par l'implémentation actuelle.
    jwtVerify.mockReturnValue({ sub: 'supabase-uid-1', role: 'anon' });
    const { context } = buildContext({ authorization: 'Bearer x' });
