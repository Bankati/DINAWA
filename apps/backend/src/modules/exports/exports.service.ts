...
function drawHeader(
  ...
  doc
    ...
    .fontSize(9)
    .fillOpacity(0.7)
    .text(rightLine, 0, 28, { align: 'right', width: 559 });
  doc.fillOpacity(1);
  doc.y = 90;
}
...
function tableHeader(
  ...
  cols.forEach((c) => {
    doc
      ...
      .font('Helvetica-Bold')
      .fontSize(8)
      .text(c.label.toUpperCase(), x + 4, doc.y + 7, {
        width: c.width - 8,
        align: c.align ?? 'left',
        lineBreak: false,
      });
...
function tableRow(
  ...
  cols.forEach((c) => {
    doc
      .fillColor(c.color ?? (isTotal ? NAVY_D : TEXT))
      .font(c.bold || isTotal ? 'Helvetica-Bold' : 'Helvetica')
      .fontSize(8.5)
      .text(c.value, x + 4, doc.y + 5, {
        width: c.width - 8,
        align: c.align ?? 'left',
        lineBreak: false,
      });
...
function drawFooter(doc: InstanceType<typeof PDFDocument>) {
  ...
  doc
    ...
    .fontSize(8)
    .fillOpacity(0.65)
    .text(`Document confidentiel · WARAH · Généré le ${fmtDate(new Date())}`, 36, y + 14, {
      align: 'center',
      width: 523,
    });
  doc.fillOpacity(1);
...
export class ExportsService {
  ...
  async paiementsPDF(user: AuthenticatedUser, from?: string, to?: string): Promise<Buffer> {
    ...
    const subtitle =
      from && to ? `Période : ${fmtDate(from)} → ${fmtDate(to)}` : `Au ${fmtDate(new Date())}`;
    ...
    payments.forEach((p, i) => {
      ...
      tableRow(
        ...
        [
          ...
          {
            value: p.paymentMethod ? (METHOD_LABELS[p.paymentMethod] ?? p.paymentMethod) : '—',
            width: 70,
          },
  ...
  async locatairesPDF(user: AuthenticatedUser): Promise<Buffer> {
    const where: Record<string, unknown> = {};
    if (user.role === 'OWNER')
      where['leasesAsTenant'] = { some: { property: { ownerId: user.id } } };
    if (user.role === 'MANAGER')
      where['leasesAsTenant'] = {
        some: { property: { mandates: { some: { managerId: user.id, status: 'ACTIVE' } } } },
      };
    ...
    tenants.forEach((t, i) => {
      const lease = (
        t as unknown as { leasesAsTenant: Array<{ property: { address: string; city: string } }> }
      ).leasesAsTenant[0];
      tableRow(
        doc,
        [
          ...
          {
            value: lease ? truncate(`${lease.property.address}, ${lease.property.city}`, 26) : '—',
            width: 160,
          },
        ],
        i,
  ...
  async rapportPDF(user: AuthenticatedUser, from?: string, to?: string): Promise<Buffer> {
    ...
    drawHeader(
      ...
      subtitle,
      `${payments.length} paiements confirmés`,
    );
    doc.moveDown(0.5);
